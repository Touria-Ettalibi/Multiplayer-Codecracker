...
- ✅ Registration, then login on the login page
- ✅ Successful login redirects to the dashboard
- ✅ Failed login stays on the page with an error message (generic, for both unknown username and wrong password)
- ✅ Logout redirects back to the login page

### User management — ✅ fulfilled

- ✅ Two roles, `ADMIN` and `USER`
- ✅ Usernames unique across all users; passwords stored as bcrypt hashes
- ✅ An admin account exists from first startup via an init script
- ✅ Only an admin sees the user-management nav item
- ✅ Admin can view, search, add, edit, and delete user accounts

### Dashboard / GUI — ⚠️ partially fulfilled

- ✅ Lobby list of currently logged-in users, updated live over WebSocket
- ✅ Header shows the logged-in user's name and role
- ✅ Any lobby player can start a game
- ✅ Everyone in the lobby is invited when a game starts
- ✅ Invited players get 30 seconds to confirm or decline
- ✅ Players who don't confirm in time don't join
- ✅ The game starts with everyone who confirmed, once at least two have
- ✅ If fewer than two confirm, the game start is cancelled
- ✅ Once started, confirmed players see their game board
- ✅ The lobby is unavailable during a game — replaced by current-game info (player names, that a game is running) for everyone else
- ⚠️ **Partial:** onlookers currently only see *that* a game is running and who's in it, not its current round number — the base requirement's "aktuelle Runde" for non-players isn't wired up (see `GAME_INVITE_FEATURE.md` for why and what it would take)
- ❌ **Not implemented:** the Highscore list (top 5 of the last 10 days) — no endpoint or UI exists for it yet. Total lifetime points per user *are* computed and shown on the per-game result screen (`GameRepository.findTotalPoints`), but nothing aggregates or ranks across users, and nothing filters to the last 10 days

### Base game configuration — ✅ fulfilled

- ✅ 4-position code, 4 colors (Rot/Blau/Grün/Gelb), colors may repeat
- ✅ At most 10 rounds per game
- ✅ 30 seconds per round to submit

### Game board — ✅ fulfilled

- ✅ Current round number shown
- ✅ Remaining time for the round shown
- ✅ Code entry; empty at round start; confirm button locks it in
- ✅ Auto-submit at timeout; an incomplete code at that point doesn't count as a valid attempt
- ✅ Players can forfeit early, counted as a loss
- ✅ A new round only starts once every active player has submitted or the round's time is up
- ✅ Own guess history with server feedback shown
- ✅ Feedback gives counts only (correct position / correct color elsewhere), never which positions
- ✅ Each secret-code position counted at most once when computing feedback (verified by `GuessEvaluator`'s dedicated duplicate-color test cases)
- ✅ Game state changes pushed over WebSocket

### Win determination — ✅ fulfilled

- ✅ Exactly one correct guess in a round → that player wins
- ✅ Multiple correct guesses in the same round → draw for those players, loss for the rest
- ✅ Nobody correct after the last round → everyone still active loses
- ✅ Every player is told the outcome and the points everyone earned

### Non-functional requirements — ✅ fulfilled

- ✅ REST + WebSocket API for the frontend
- ✅ Private endpoints require a valid JWT; role-restricted endpoints (user management) additionally check the role claim
- ✅ Invalid/expired tokens are rejected — `401` for REST, connection refused before upgrade for WebSockets
- ✅ REST responses use appropriate HTTP status codes throughout (see the tables above)
- ⚠️ JavaDoc is present throughout the backend; TSDoc coverage on the frontend is inconsistent — worth a pass before submission

### Optional bonus tasks — ❌ not implemented

- ❌ Difficulty levels (more colors, configurable round time) — `GameService` fixes both as constants; its own Javadoc notes `GuessEvaluator` already supports arbitrary length/color sets, so this wouldn't require touching the evaluation logic itself
- ❌ Lobby chat between players
- ❌ Spectator mode for players not in the running game
- ❌ Multiple simultaneous games — `GameInviteService` explicitly limits the whole lobby to one invite/game at a time
